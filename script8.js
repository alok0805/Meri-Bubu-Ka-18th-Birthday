...
document.getElementById('s2uBtn').addEventListener('click', function () {
  const cryDiv = document.getElementById('cryEffect');
  cryDiv.innerHTML = '';
  cryDiv.style.display = 'block';

  // Create animated crying emoji drops
  for (let i = 0; i < 18; i++) {
    const drop = document.createElement('div');
    drop.textContent = '😭';
    drop.style.position = 'absolute';
    drop.style.left = (10 + Math.random() * 80) + 'vw';
    drop.style.top = (-10 - Math.random() * 10) + 'vh';
    drop.style.fontSize = (2.2 + Math.random() * 1.2) + 'rem';
    drop.style.opacity = 0.85;
    drop.style.animation = `cryDrop 1.6s ${Math.random() * 0.7}s ease-in forwards`;
    cryDiv.appendChild(drop);
  }

  setTimeout(() => {
    cryDiv.style.display = 'none';
    cryDiv.innerHTML = '';
  }, 2200);
});
